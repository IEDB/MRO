...
def get_SLA_sequences():
    """Gets SLA sequences from IPD-MHC database"""
    ...
    with open(sys.argv[6]) as fasta:
        ...
        for line in fasta:
            if line.startswith(">"):
                ...
                # Match the accession
                if line.startswith(">IPD-MHC:SLA") and "SLA" in line.split(" ")[1]:
...
def get_current_loci():
    """Simple method to get current SLA loci in MRO"""
    mro_loci = set()
...
def update_molecules(missing_alleles):
    """Builds and updates SLA molecules"""
    class_I_genes = ["1", "2", "3", "6"]
    class_II_genes = ["DRA", "DRB1", "DRB2", "DRB3", "DRB4", "DRB5", "DQA", "DQB1"]
...
def update_IEDB_tab(missing_molecules):
    ...
    with open(sys.argv[7], "a+") as outfile:
        ...
        for molecule in missing_molecules:
            ...
            curr_iedb_id += 1
            # Specific to SLA alleles, either DR, DP or whatever locus is
            if "DR" in locus:
...
ipd_seqs, allele_map = get_SLA_sequences()
