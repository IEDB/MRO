...
def get_taxon_metadata(metadata):
    ...
    for species in metadata["species"]:
        ...
        for gene in species["pairing"].keys():
            ...
            taxon_metadata[alpha] = taxon
            taxon_metadata[beta] = taxon
...
def create_classII_pairing(allele, pairings, pairings_rev):
    ...
    """
    if allele in pairings.keys():
        pair = f"{allele}/{pairings[allele]}"
    else:
        pair = f"{pairings_rev[allele]}/{allele}"
...
def update_molecules(
    ...
    for x in list(create_classI_prot(classI)):
        if x not in mro_proteins:
            new_molecules.add(x)
    for y in list(create_classII_prot(classII, pairings, pairings_rev)):
...
curr_loci = get_current_loci()
missing_alleles = update_chains(curr_loci, ipd_seqs, allele_map, metadata, missing_loci)
